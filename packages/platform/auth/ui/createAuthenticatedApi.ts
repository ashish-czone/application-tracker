...
import { authEvents, SESSION_EXPIRED_EVENT } from './authEvents';
...
async function attemptRefresh(baseUrl: string): Promise<boolean> {
  ...
  try {
    ...
    const res = await fetch(`${baseUrl}/auth/client/refresh`, {
      method: 'POST',
...
/**
 * Creates an auth-aware HTTP client that automatically attaches Bearer tokens,
 * handles silent token refresh on 401, and dispatches session expiry events.
 */
export function createAuthenticatedApi(baseUrl: string): ApiFn {
  async function request<T>(method: string, path: string, body?: unknown): Promise<T> {
    ...
    if (res.status === 401 && accessToken) {
      if (!refreshPromise) {
        refreshPromise = attemptRefresh(baseUrl).finally(() => {
          refreshPromise = null;
        });
      }
