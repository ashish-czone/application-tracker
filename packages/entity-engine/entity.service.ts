...
export class EntityService {
  ...
  async clone(id: string, actorId: string): Promise<Record<string, unknown>> {
    ...
    // 3. Build clone payload — iterate field defs, skip system/auto/workflow fields
    ...
    for (const def of defs) {
      ...
      if (def.fieldType === 'tags') {
        // Hydrated as [{id, name, color}] → need string[]
        if (Array.isArray(value)) {
          clonePayload[def.fieldKey] = value.map((t: any) => t.id);
        }
        continue;
      }

      if (def.fieldType === 'multi_user' || def.fieldType === 'multi_lookup') {
        // Hydrated as [{id, label}] → need string[]
        if (Array.isArray(value)) {
          clonePayload[def.fieldKey] = value.map((v: any) => v.id);
        }
        continue;
      }
      ...
      if (def.fieldKey === primaryNameField && typeof value === 'string') {
        clonePayload[def.fieldKey] = `Copy of ${value}`;
        continue;
      }

      clonePayload[def.fieldKey] = value;
    }
