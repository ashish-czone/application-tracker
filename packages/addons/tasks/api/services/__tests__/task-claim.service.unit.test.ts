...
describe('TaskClaimService', () => {
  ...
  describe('pickup', () => {
    ...
    it('throws BadRequest when task has no team (not a pickable task)', async () => {
      mockDb._select.where.mockResolvedValueOnce([
    ...
    it('returns the pickup when user is a team member and task is un-picked-up', async () => {
      mockDb._select.where.mockResolvedValueOnce([
        { id: 'task-1', assigneeId: null, assigneeTeamId: 'team-1' },
      ]);
      mockOrgUnits.getMemberIds.mockResolvedValueOnce(['user-1']);
  ...
  describe('reassign', () => {
    it('throws BadRequest when neither userId nor teamId provided', async () => {
      await expect(service.reassign('task-1', {})).rejects.toThrow(BadRequestException);
    });

    it('throws BadRequest when both userId and teamId provided', async () => {
      await expect(
        service.reassign('task-1', { userId: 'user-1', teamId: 'team-1' }),
      ).rejects.toThrow(BadRequestException);
    });

    it('throws BadRequest when task does not exist', async () => {
      mockDb._select.where.mockResolvedValueOnce([]);
      await expect(service.reassign('task-1', { userId: 'user-1' })).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('assigns to user and clears team', async () => {
      ...
      const result = await service.reassign('task-1', { userId: 'user-1' });
  ...
  describe('kind guard', () => {
    ...
    it('reassign runs input validation before the guard (missing both ids)', async () => {
      registerTasksKindLookup(async () => 'compliance');
