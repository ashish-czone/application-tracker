 ...
 * position) cannot function for an app that uses org-units. The library
 * ships only classes — each app composes its own NestJS module.
 *
...
/**
 * Seed source list for apps that use org-units. org-units is an addon,
 * so it is not included in `platformSystemSeedSources()`. Apps that
