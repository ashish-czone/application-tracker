...
export class JobOpeningsService {
  constructor(
    @Inject('ENTITY_SERVICE_job_openings') private readonly entityService: EntityService,
  ) {}
  ...
  create(input: CreateJobOpeningDto, actorId: string) {
    return this.entityService.create(input, actorId);
  }

  update(id: string, input: UpdateJobOpeningDto, actorId: string, accessCtx?: DataAccessContext) {
