...
export function LawsLibraryPage() {
  ...
  // Once data lands, default the active selection to the first root.
  useEffect(() => {
      ...
      <main className="max-w-[1480px] mx-auto px-10 py-8">
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          <div className="border border-rule bg-paper-raised px-4 py-3">
            ...
            <div className="mt-1 font-mono text-2xl text-ink tabular-nums">{tree.length}</div>
          </div>
          <div className="border border-rule bg-paper-raised px-4 py-3">
            <Eyebrow>Central</Eyebrow>
            <div className="mt-1 font-mono text-2xl text-ink tabular-nums">
              {jurisdictionCounts.central}
            </div>
          </div>
          <div className="border border-rule bg-paper-raised px-4 py-3">
            <Eyebrow>State</Eyebrow>
            <div className="mt-1 font-mono text-2xl text-ink tabular-nums">
              {jurisdictionCounts.state}
            </div>
          </div>
          <div className="border border-rule bg-paper-raised px-4 py-3">
            <Eyebrow>Municipal</Eyebrow>
            <div className="mt-1 font-mono text-2xl text-ink tabular-nums">
              {jurisdictionCounts.municipal}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)] gap-6">
          <section className="border border-rule bg-paper-raised">
            ...
            <div>
              {isLoading && (
                <div className="px-4 py-6 text-[11px] text-ink-muted font-sans">Loading laws…</div>
              )}
              {isError && (
                <div className="px-4 py-6 text-[11px] text-signal font-sans">
                  Failed to load laws. Refresh to retry.
                </div>
              )}
              {!isLoading && !isError && tree.length === 0 && (
                <div className="px-4 py-6 text-[11px] text-ink-muted font-serif italic">
                  No laws yet. Use “New law” to add the first one.
                </div>
              )}
              {tree.map((node) => (
                <LawTreeRow
            ...
            <aside className="border border-rule bg-paper-raised h-fit sticky top-6">
              ...
              <div className="px-5 py-4 border-b border-rule">
                ...
                <div className="mt-1 flex items-end gap-2">
                  <span className="font-mono text-2xl text-ink tabular-nums">—</span>
