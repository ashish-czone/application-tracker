...
- [ ] **I22.** Guard on `org_units` delete: reject if any `law_handlers` row references the unit. **Deferred** — `org_units` lives in `packages/addons/org-units` (domain-agnostic) but the guard requires compliance-domain knowledge. No domain logic in packages, no callback registries (per established users-as-library precedent). Tracked as a follow-up org-units-as-library stream that mirrors PRs #1027/#1028: (1) reshape `@packages/org-units` to library-only — drop `@Module/@Global`, expose classes + factory, each app wires its own `OrgUnitsModule`; (2) add a protected `assertCanDelete()` template method + `ComplianceOrgUnitService` subclass in `apps/compliance` that queries `law_handlers`. I22 falls out as ~10 lines in the second PR. (Q13)
