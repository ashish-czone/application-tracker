...
**I22 shipped 2026-04-27** via PR #1071 on top of org-units-as-library reshape #1070. `ComplianceOrgUnitService` overrides `OrgUnitService.assertCanDelete()` and counts `compliance_law_handlers` rows referencing the unit, throwing `BadRequestException { code: 'HANDLERS_REFERENCE_UNIT' }` when > 0. Stream I now fully closed.
...
- **`canResolveAssignee` over `hasResolvableHandler`.** Resolver method is `resolveAssignee`, predicate noun should match. Co-located on `ComplianceRulesService` (where the resolver already lives) rather than `LawHandlersService` (where `hasDefaultHandler` lives) — sister methods of the same operation cluster together. Both consume a shared private `findResolvedHandler` returning `{ kind: 'resolved'|'ambiguous'|'none' }`, so `resolveAssignee`'s existing throw semantics are preserved bit-for-bit and `excludeHandlerId` falls out for free.
- **Dropped `firmId` param** spec'd in the original todo. Codebase uses implicit tenancy via `getTenantId()` from logger context; explicit `firmId` param creates drift since callers would derive a value just to pass it through. Signature is `canResolveAssignee(lawId, clientId?, excludeHandlerId?)`.
- **I22 deferred to a parallel org-units-as-library stream.** `org_units` lives in `packages/addons/org-units` (domain-agnostic) but the I22 guard requires compliance-domain knowledge. Hooks/callbacks rejected (don't abstract away the details). Forking into compliance rejected (recruit + future apps want org-units the same way; ~95% of org-units is library-worthy, only `delete()` policy isn't). Decision: mirror PRs #1027/#1028 — reshape `@packages/org-units` to library-only, add a protected `assertCanDelete()` template method, compliance app subclasses. I22 then falls out as ~10 lines in the second PR. **Shipped 2026-04-27 via #1070 (reshape) + #1071 (I22 subclass).**

I21 wiring detail: guard at `LawHandlersController.delete()` rather than service layer — DELETE has a single entry point unlike I20's three create paths. `ComplianceRulesService.assertHandlerCanBeDeleted` reads `complianceClientRegistrations` directly (rather than via `ClientRegistrationsService`) to avoid a circular service dep — the registrations service already injects rules for I20. `forwardRef` on the `LawHandlersModule ↔ ComplianceRulesModule` pair resolves the structural cycle Nest now sees; standard tool for this graph.

I23 deep-link target: `/laws` for now — no dedicated `/law-handlers` admin page in V1. Updating the target is a one-line change when a proper handler-admin page lands. The detection helper (`isNoResolvableAssigneeError`) and code constant (`NO_RESOLVABLE_ASSIGNEE`) live in `useClientsApi.ts` so other drawers (client-detail "Add registration", future bulk import) reuse them without re-deriving the body shape. Banner is one-shot, cleared on every submit.

Tests: 6 + 4 + 5 = 15 new unit tests across rules and registrations services. Integration tests skipped — broken since #1002 (memory `project_compliance_integration_tests_di.md`); not adding to a red baseline.
...
- [x] **A6.** Two parts:
  - **`allowedActions` on list/get responses** — shipped 2026-04-27. New `TaskActionsService` in `@packages/tasks` computes the set of action verbs the requesting user can currently perform on each task — CRUD verbs (`update`, `delete`, `clone`, `restore`), claim verbs (`pickup`, `unclaim`, `reassign`), and workflow transitions (`transition:<target>`). `TasksController.list` and `TasksController.findOne` now thread the JWT user into `TasksService` so each row is annotated with `allowedActions: string[]`. Bulk variant batches team-membership lookups per unique team to avoid N+1; skips the prefetch entirely when the caller lacks `tasks.pickup`. Authoritative checks remain on the action endpoints — the verb list is a UI affordance hint, not a security boundary.
  - **Scope-on-position integration end-to-end** — closed by virtue of Streams B, C, and I shipping. Stream B's three user-resolver strategies (`org_unit_head`, `parent_unit_head`, `org_unit_members`) walk the position → scope → descendants chain in production for every overdue/T+3/T+7 escalation. Stream C seeded the default positions/scopes those resolvers consume. The integration is now battle-tested without a separate verification PR. (Q3, Q4)
...
- [x] **H6.** UI "Deactivate" button with consequence summary — shipped 2026-04-27. `UsersListPage` confirmation modal renamed from "Delete user" to "Deactivate user"; description corrected (was "permanently delete... all associated data" — both bits were wrong, since the operation is soft-delete with a Restore path); consequence bullets added under the description: removed from team memberships, active task assignments cleared (tasks remain on their teams), cannot sign in. Toast messages and aria-labels updated to match. Platform-level wording — domain-agnostic, applies to every app that uses the users module.
...
- [x] **I19.** Service helper `canResolveAssignee(lawId, clientId?, excludeHandlerId?)` on `ComplianceRulesService` — boolean twin of `resolveAssignee`, returns true iff the 4-tier walk would yield exactly one handler. Both methods consume a shared private `findResolvedHandler` helper that returns a discriminated `{ kind: 'resolved'|'ambiguous'|'none' }` result; the throwing semantics of `resolveAssignee` are preserved bit-for-bit. Renamed from todo's `hasResolvableHandler` — `resolveAssignee` is the existing verb, the predicate noun should match. Dropped `firmId` from the spec'd signature: tenancy is implicit via `getTenantId()` in the codebase, an explicit param creates drift. `excludeHandlerId` simulates removing a row before resolving — used by I21. Ambiguous tiers count as not resolvable: a registration whose generation would later throw on ambiguity is just as broken as one with no handler. (Q13)
- [x] **I20.** Guard on `client_registrations` create: rejects with `NoResolvableAssigneeError` (400 `NO_RESOLVABLE_ASSIGNEE`, `lawId: string`) when `canResolveAssignee` returns false. Wired at `ClientRegistrationsService` across all three create paths — generic CRUD `create()`, the domain `register()` verb, and the `registerMany()` batch — so the rejection surfaces consistently regardless of entry point. The batch validates each law upstream of the transaction so a single missing handler rejects the whole batch atomically. `ComplianceRulesService` is injected via the new `ComplianceRulesModule` import in `ClientRegistrationsModule`. (Q13)
- [x] **I21.** Guard on `law_handlers` delete: rejects with `LawHandlerRequiredError` (400 `LAW_HANDLER_REQUIRED`, `affectedRegistrationCount: number`) when removal would leave at least one active registration without a resolvable assignee. `ComplianceRulesService.assertHandlerCanBeDeleted(handlerId)` walks active registrations on the handler's law and runs `canResolveAssignee` per registration with the row excluded. Wired at `LawHandlersController.delete()` rather than the service layer — DELETE has a single entry point unlike I20's three create paths, so the controller is the right seam. The active-registrations read goes direct to `complianceClientRegistrations` rather than via `ClientRegistrationsService` to avoid a circular service dep (the registrations service already injects rules for I20). `forwardRef` on the `LawHandlersModule ↔ ComplianceRulesModule` pair resolves the structural cycle Nest now sees. (Q13)
- [x] **I22.** Guard on `org_units` delete: reject if any `law_handlers` row references the unit. Shipped via the org-units-as-library stream — PR #1070 reshaped `@packages/org-units` to library-only (mirrors users-as-library #1027), and this PR adds a `protected assertCanDelete()` template method on `OrgUnitService` (called inline at the top of `delete()`) plus `apps/compliance/src/modules/org-units/compliance-org-unit.service.ts` which overrides it to count `compliance_law_handlers` rows where `orgEntityId = id`. Throws `BadRequestException` with `code: 'HANDLERS_REFERENCE_UNIT'` and the affected count when > 0. Compliance's app-level `OrgUnitsModule` binds `OrgUnitService` to `ComplianceOrgUnitService` so every `OrgUnitController.delete()` call in the compliance app runs through the guard. (Q13)
- [x] **I23.** UI on registration creation: `NewClientDrawer` surfaces the I20 rejection inline above the law-selection section with a deep link to `/laws`, instead of a generic toast. Detection lives in the API hook layer as `isNoResolvableAssigneeError` + the `NO_RESOLVABLE_ASSIGNEE` constant exported from `useClientsApi`, so other drawers (client-detail "Add registration", future bulk import) can reuse it without re-deriving the body shape. The mutation hook suppresses the toast for this code so the inline banner is the only surface. Deep link target is the laws library — there's no dedicated `/law-handlers` admin page in V1; updating the link is a one-line change when a proper handler-admin page lands. Banner is one-shot, cleared on every submit so a successful retry doesn't leave a stale message; per-law enumeration deferred since the backend rejects on the first missing-handler law in the batch. (Q13)
