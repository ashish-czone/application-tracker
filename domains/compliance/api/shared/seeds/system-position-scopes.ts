...
/**
 * System seed for (position × work-entity) scope rows.
 *
 * Per Q14 (see domains/compliance/todos.md §1), V1 seeds scopes only for
 * work entities — other compliance entities (clients, registrations, rules,
 ...
 *
 * Ten rows total (5 positions × 2 entity slugs):
 *
 *   position       | tasks       | compliance-filings
 *   ---------------|-------------|--------------------
 *   Member         | unit        | unit
 *   Lead           | unit        | unit
 *   Head           | unit        | unit
 ...
 * — `entityType = model.slug`). The filings slug is hyphenated.
 *
...
const WORK_ENTITY_TYPES = ['tasks', 'compliance-filings'] as const;
...
export const seedSystemPositionScopes = async (ctx: INestApplicationContext): Promise<void> => {
  ...
  for (const [positionName, scope] of Object.entries(SCOPE_BY_POSITION)) {
    ...
    for (const entityType of WORK_ENTITY_TYPES) {
