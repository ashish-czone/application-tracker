...
- **Don't accept workflow state fields on create or update DTOs.** Workflow state is system-managed: the workflow definition is the only description of which states an entity can be in and how it gets there. Creates always start at `WORKFLOW.initialState` (set unconditionally by the service); state changes go only through `POST /<entity>/:id/transition`, which enforces guards, `requiredPermissions`, `reasonRequired`/`commentRequired`, and writes `workflow_transition_history`. The moment the create DTO picks the state field, every gate the transition path provides is bypassable by anyone with `<entity>.create`. Bulk imports / migrations / seeds use a separate, explicit path (`cli/seed.ts` direct DB writes, or an admin-only `POST /admin/<entity>/import` behind `*`) — never the regular create endpoint with a flag. The response MAY expose an external alias for the storage column (e.g. response `status` ↔ column `complianceStatus`); the alias lives only in the response. Full rule: `.claude/rules/workflow-entity-creates.md`.
