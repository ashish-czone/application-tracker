...
export default async function globalSetup(): Promise<void> {
  ...
  if (res.status === 429) {
    const waitMs = 65_000;
    console.log(`[e2e-agency-admin globalSetup] 429 throttled; waiting ${waitMs / 1000}s then retrying.`);
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `e2e-agency-admin globalSetup: login failed (${res.status}) for ${ADMIN_EMAIL}. ` +
