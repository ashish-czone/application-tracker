...
packages/platform/*  →  packages/core/* + other platform packages
                        EXCEPTIONS: app-shell may import from addons (it is the
                        runtime integrator that wires every package into a
                        runnable app); platform-testing may import from addons
                        (it is the test integrator that bootstraps a test app
                        with every relevant addon wired in)
...
**`packages/platform/app-shell` and `packages/platform/testing` are the documented exceptions** to the platform → addon rule. Both are integrators with no kernel-tier code of their own:
- `app-shell` wires every module (core + platform + addon + domain) into a runnable `NestModule` / `AppRouter`.
- `platform-testing` exposes `createPlatformTestModule` / `createTestApp` — the central test bootstrap that imports addons (entity-engine, workflows, automations mocks, etc.) so domain and addon test suites get a consistent integration-test environment without re-wiring the world.

No other platform package may import from addons.
