...
export class ClientsController {
  ...
  @Get('layout/list') @RequirePermission('clients.read')
  ...
  @Get(':id') @RequirePermission('clients.read')
  findOne(@Param('id', ParseUUIDPipe) id: string, @AccessContext() c?: DataAccessContext) { return this.svc.findOne(id, c); }

  @Post() @RequirePermission('clients.create') @HttpCode(HttpStatus.CREATED)
  create(@Body() body: unknown, @CurrentUser() u: JwtPayload) { return this.svc.create(CreateClientSchema.parse(body), u.userId); }

  @Patch(':id') @RequirePermission('clients.update')
  update(@Param('id', ParseUUIDPipe) id: string, @Body() body: unknown, @CurrentUser() u: JwtPayload, @AccessContext() c?: DataAccessContext) {
    return this.svc.update(id, UpdateClientSchema.parse(body), u.userId, c);
  }

  @Delete(':id') @RequirePermission('clients.delete') @HttpCode(HttpStatus.NO_CONTENT)
  ...
  @Post(':id/clone') @RequirePermission('clients.create') @HttpCode(HttpStatus.CREATED)
  clone(@Param('id', ParseUUIDPipe) id: string, @CurrentUser() u: JwtPayload) { return this.svc.clone(id, u.userId); }

  @Post(':id/restore') @RequirePermission('clients.update')
